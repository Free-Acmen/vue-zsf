(function(){
    var usitrip = window.usitrip = window.usitrip || {};
    usitrip.staticResource = usitrip.staticResource || {};
    ...
    //创建背景层状态码
    /*  注意：为应对特殊情境：可在实例处创建背景层  类名=>newMobilePhoneBg */
    window.usitrip_crt_mobilePhoneBg = true;
    window.usitrip_crt_mobilePhoneBgClass = 'newMobilePhoneBg';
...
function MobilePhone(opt) {
     ...
     */
    this.areaEnDisplay = false;

    /**
     * 是否显示下拉列表中国家区号
     * @cfg {boolean}
     */
    this.areaDdCodeDisplay = false;

    /**
     * 默认的提示信息
     * @cfg tipText {String}
     */
    this.tipText = '请输入正确格式的号码,格式为:+86 17012345678';
...
MobilePhone.prototype = {
    init : function(opt) {
        this.el = opt.el || this.el;
        this.name = opt.name || this.name;
        this.areaname = opt.areaname || this.areaname;
        this.hotClass = opt.hotClass || this.hotClass;
        this.commonClass = opt.commonClass || this.commonClass;
        this.initValue.phoneValue = opt.initValue.phoneValue || this.initValue.phoneValue;
        this.initValue.phoneId = opt.initValue.phoneId || this.initValue.phoneId;
        this.initValue.zhWidth = opt.initValue.zhWidth || this.initValue.zhWidth;
        this.initValue.phoneWidth = opt.initValue.phoneWidth || this.initValue.phoneWidth;
        this.initValue.placeholder = opt.initValue.placeholder || this.initValue.placeholder;
        this.tipson = opt.tipson || this.tipson;
        this.areaEnDisplay = opt.areaEnDisplay || this.areaEnDisplay;
        this.areaDdCodeDisplay = opt.areaDdCodeDisplay || this.areaDdCodeDisplay;
        this.tipText = opt.tipText || this.tipText;
        this.areaAttr = opt.areaAttr || this.areaAttr;
        this.phoneAttr = opt.phoneAttr || this.phoneAttr;
        this.border = opt.border || this.border;
        this.needIco = opt.needIco || this.needIco;
        this.initData();
        this.initBg();
        ...
        if(!this.areaDdCodeDisplay){
            this.areaDdCode.css('display', 'none');
        }

        this.setBorder(this.border);
        //alert(this.areaPhone.width() + "\r\n" + this.areaPhoneCode.width());
    },
     ...
     */
    initData: function(){
        // 处理 "+86 +86 1234567" 的情况
        if(/^(\+\d+\s){2}/.test(this.initValue.phoneValue)){
            this.initValue.phoneValue = this.initValue.phoneValue.replace(/^(\+\d+\s){1}/,'');
        }
    ...
    render: function(el){
        ...
        var phoneInit = phone;
        var phoneInit_val = phone ? '+'+code+' '+phone : '+'+code;  //隐藏域 input[phone]

        this.origin = zh;

        var content = '<div class="area-wrap-two">';
        content += '<div class="con clearfix">';
        /*content += '<div class="search"><input class="search_ipt" type="hidden" value=""/><span class="done_btn">完成</span></div>';*/
        content += '<div class="search"><p><span class="title">请选择手机号码归属地：</span><span class="done_btn">完成</span></p></div>';
        content += '<dl class="area-con-dl">';
        content += this.getListHtml(this.hots, this.hotClass);
        content += '<dd class="spl"><div class="spl-up"></div><div class="spl-center"></div><div class="spl-down"></div></dd>'
        content += this.getListHtml(this.common, this.commonClass);
        content += '</dl></div>';
        content += '<div class="area-box"><div class="areas"><i class="flag"';
        if (this.needIco == false) {
            content += ' style="display:none"';
        }
        content += '></i><input name="' + this.areaname + '" type="hidden" value="+' + code + '"><span class="areaname">' + zh + '</span><em><!– –></em></div>';
        content += '<div class="phone-box"" style="';
        if(this.initValue.phoneWidth) {
            content += 'width:' + this.initValue.phoneWidth + 'px;';
        }
        content += '"><span class="JS-code">+' + code + '</span><input class="phone" type="tel" value="' + phoneInit + '" placeholder="'+this.initValue.placeholder+'" /><input type="hidden" name="' + this.name + '" value="' + phoneInit_val + '" class="phone_val" id="'+this.initValue.phoneId+'"/><span class="JS-tips"';
        if(phoneInit){
            content += ' style="display:none"';
        }
        content += '>' + this.initValue.placeholder + '</span></div><i class="tips">' + this.tipText +'</i></div>'
            + '</div>';
        $(el).html(content);
    },
    ...
    doEvent: function(){
        var el = this.el;
        this.areaWrap = $(el).find('.area-wrap-two');
        this.areaWrapCon = this.areaWrap.find('.con');
        this.areaSearchIpt = this.areaWrapCon.find('input.search_ipt');
        this.areaSelectDoneBtn = this.areaWrapCon.find('span.done_btn');
        this.areaWrapDl = this.areaWrapCon.find('dl');
        this.areaWrapDd = this.areaWrapDl.find('dd:not(.spl)');
        ...
        this.areaDdCode = this.areaWrapDd.find('em');
        ...
        this.areaCode = this.areas.find('input[name="' + this.areaname + '"]');
        // this.areaBoxEm = this.areas.find('em');
        this.areaPhone = this.areaBox.find('.phone');
        this.areaTips = this.areaBox.find('.tips');
        this.phoneBox = this.areaBox.find('.phone-box');
        this.areaPhoneCode = this.phoneBox.find('.JS-code');
        this.areaPhoneTips = this.phoneBox.find('.JS-tips');

        // 手机号输入提示SPAN层
        this.areaPhoneTips.click($.proxy(function(){
            this.areaPhoneTips.css('display','none');
            this.areaPhone.focus();
        },this));
        // 国际区号被点 要显示有焦点
        this.areaPhoneCode.click($.proxy(function(){
            this.areaPhoneTips.css('display','none');
            this.areaPhone.focus();
        },this));
        // 手机号输入框有焦点了，就要隐藏提示层
        this.areaPhone.focus($.proxy(function(){
            this.areaPhoneTips.css('display','none');
        },this));
        // 手机号输入框失去焦点时，如果为空，则显示提示层
        this.areaPhone.blur($.proxy(function(){
            if(this.areaPhone.val().replace(/^\s+|\s+$/g,'') == '') {
                this.areaPhoneTips.css({'display':'','left':this.areaPhoneCode.outerWidth(true)+'6'});
            }else {
                var value = this.areaPhone.val().replace(/^\s+|\s+$/g,'');
                if(/^\+/.test(value)){
                    var val = value.split(' ');
                    var code = this.findAreaPhoneByCode(val[0]);
                    this.areaPhone.val(val[1]);
                    this.areaCode.val('+'+code[2]);
                    this.areaPhoneCode.text('+'+code[2]);
                    this.areaName.text(code[0]);
                    this.updateBgImgPosition(this.areaFlag, code[2], code[0]);
                    value = val[1] ? val[1] : "";
                }
                this.areaPhone.val(value.replace(/%|'/g,'').replace(/[^0-9*]/ig,''));
            }
            this.areaPhone.siblings(".phone_val").val(this.areaCode.val() +' '+this.areaPhone.val());
        },this));

        // 显示国家选择层
        this.areas.click($.proxy(function(e){
            e = e || event;
            this.show();
            e.stopPropagation();
        },this));
        ...
        //选择国家的层，禁止事件冒泡
        this.areaWrapCon.click(function(e){
            e = e || event;
            e.stopPropagation();
        });
        ...
        // 点选国家
        var that = this;
        this.areaWrapDd.click(function(e){
            e = e || event;
            e.stopPropagation();
            var obj = $(this);
            ...
            var zh = $(currentEl).find('span[lang=zh]').text(),
                en = $(currentEl).find('span[lang=en]').text(),
                code = $(currentEl).find('em').text();

            that.updateBgImgPosition(that.areaFlag, code, zh);
            that.areaName.text(zh);
            that.areaCode.val(code);
            that.areaPhoneCode.text(code);
            that.areaPhone.siblings(".phone_val").val(that.areaCode.val() +' '+that.areaPhone.val());
            that.hide();
        });

        // 输入筛选
        $(document).on('input propertychange','.area-wrap-two input.search_ipt',function (e) {
            if (e.type === "input" || e.orignalEvent.propertyName === "value") {
                var value = this.value.replace(/^\s+|\s+$/g,'');
                if(value) {
                    var code = that.findAreaPhoneByInput(value);
                    console.log(code);
                }
            }
        });
        ...
        // 完成 按钮事件
        this.areaSelectDoneBtn.click($.proxy(this.hide, this));
        var keyDownAction = function(e){
            if(this.areaWrapDl.is(':hidden')) {
                return;
            }
            e = e || window.event;
            e.stopPropagation();
        ...
        //$(document).keydown($.proxy(keyDownAction,this));
    ...
    initDefaultValue: function(){
        ...
        if(_clientWidth >=640 ){
            this.initValue['zhWidth'] = this.initValue['zhWidth'] * 2;
        }
        this.areaName.css({
            width: this.initValue['zhWidth']
        });

        this.areaPhone.width('73%');
    ...
    getListHtml: function(data, clazzName){
        ...
        for(var i=0,len=data.length;i<len;i++){
            ...
            if(special.indexOf(code) > -1){
                ...
                html += '<dd class=' + clazzName + '><b class="radio-ico"></b><input type="radio" id="radio' + i + '" name="radio" /><label for=""><i style="background-position:' + position + ';';
                if (this.needIco == false) {
                    html += 'display:none;';
                }
                html += '"></i><span lang="zh">' + name + '</span><span lang="en" zh="' + attr['zh'] + '">' + en + '</span><em>+' + code + '</em></label></dd>';
            } else {
                html += '<dd class=' + clazzName + '><b class="radio-ico"></b><input type="radio" id="radio' + i + '" name="radio"/><label for=""><i style="background-position:' + this.areaBgPosition[attr['code']] + ';';
                if (this.needIco == false) {
                    html += 'display:none;';
                }
                html += '"></i><span lang="zh" zh="' + attr['zh'] + '">' + attr['zh'] + '</span><span lang="en">' + en + '</span><em>+' + attr['code'] + '</em></label></dd>';
            }
        }
        return html;
    },
    ...
    show: function(){
        ...
        var body = $('body');
        body.css({'height':'100%','position': 'fixed','top' : body.scrollTop() * -1,'left':0,'right':0});
        this.areaWrapDd.removeClass('hover');
        this.areaWrapDl.find('span[zh=' + this.areaName.text() + ']').parents('dd').addClass('hover').find('input').attr('checked',true);
        $('.'+usitrip_crt_mobilePhoneBgClass).show();
        this.areaWrapCon.show();
        this.visibility = true;
    },

    findAreaPhoneByInput : function(value){
        var obj = this.areaWrapDl.find('span:contains("' + value + '")').parents('dd');
        if(obj.length == 0) {
            obj = this.areaWrapDl.find('em:contains("' + value + '"):first').parents('dd');
        }
        if (obj.length > 0) {
            var top = this.areaWrapDl.offset().top;
            var ddTop = obj.eq(0).offset().top;
            console.log(ddTop - top);
        }
    },
    ...
    hide: function(){
        ...
        scrollTop = Math.abs(scrollTop.replace(/px/,''));
        body.css({'height':'','position': '','top':''});
        body.scrollTop(Math.abs(scrollTop));
        $('.'+usitrip_crt_mobilePhoneBgClass).hide();
     ...
     */
    initBg:function () {
        if(usitrip_crt_mobilePhoneBg){
            var bgNode = document.createElement("div");
            bgNode.setAttribute("class",usitrip_crt_mobilePhoneBgClass);
            document.body.appendChild(bgNode);
            usitrip_crt_mobilePhoneBg = false;
        }
    }
...
}else if(typeof define === "dunction" && define.amd){
    define(function(){return MobilePhone})
}else{
